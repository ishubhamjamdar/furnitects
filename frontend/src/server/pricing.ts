import type { Customization, DesignType, Dimensions, PriceBreakdown } from '@/lib/types';

const REFERENCE_VOLUME_CM3 = 210 * 120 * 60;
...
function computeCustomizationCharge(customization?: Customization): number {
  ...
  let charge = 0;
  if (customization.materials && customization.materials !== 'standard') charge += 5000;
  if (customization.finish && customization.finish !== 'standard') charge += 3000;
  if (customization.addOns?.length) charge += customization.addOns.length * 2500;
  if (customization.description?.trim()) charge += 4000;
  return charge;
}
...
export function calculateQuote(
  ...
  const volumeRatio = computeVolumeCm3(dimensions) / REFERENCE_VOLUME_CM3;
